...
fn main() {
    ...
    // Benchmark serialize_batch_with_keys
    // Run a few times to warmup
    for _ in 0..3 {
        let _ = serializer.serialize_batch_with_keys(items.clone()).unwrap();
    }

    let start = Instant::now();
    let _ = serializer.serialize_batch_with_keys(items).unwrap();
    let duration = start.elapsed();

    println!("serialize_batch_with_keys optimized ({} items): {:?}", num_items, duration);
}
