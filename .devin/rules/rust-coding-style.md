...
paths:
  - "**/*.rs"
---
# Rust Coding Style for Wireframe-AI

Wireframe-AI specific Rust coding conventions and standards.
...
**Wireframe-AI Topic Naming:** NATS topics use `namespace.noun.verb` or `namespace.noun` format, lowercase, dot-separated.
...
Wireframe-AI modules are organized by domain:
...
│   │   ├── scheduler.rs
│   │   └── dispatcher.rs
└── sandbox/        # Sandbox execution module
    ├── src/
    │   ├── lib.rs
    │   └── executor.rs
```
...
Never change root fields in message envelopes — only modify payload:

```rust
// GOOD — modify only payload, preserve envelope metadata
pub fn process_message(mut envelope: MessageEnvelope) -> MessageEnvelope {
    envelope.payload = transform_payload(envelope.payload);
    envelope
}

// BAD — modifying root fields breaks compatibility
pub fn process_message_bad(mut envelope: MessageEnvelope) -> MessageEnvelope {
...
- Publish to `sys.module.online` on startup
- Publish to `sys.module.offline` on shutdown

```rust
// Module startup
async fn start_module(&self) -> anyhow::Result<()> {
    self.nats.publish("sys.module.online", &self.module_info).await?;
    // ... module initialization
}

// Module shutdown
async fn stop_module(&self) -> anyhow::Result<()> {
...
Wireframe-AI uses Tokio for async runtime:

- Use `#[tokio::main]` for async main functions
- Prefer `tokio::spawn` for concurrent task execution
- Use `tokio::select!` for multi-way select on async operations
- Always handle cancellation with `tokio::select!` or `CancellationToken`
...
See skill: `rust-pro` for comprehensive Rust idioms and patterns.
See skill: `async-tokio-patterns` for async/await patterns in Wireframe-AI.